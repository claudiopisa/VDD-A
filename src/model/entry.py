...
class Entry(ABC):
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if "_length_cache" not in cls.__dict__:
            ...
            raise TypeError(f"Subclass {cls.__name__!r} of Entry must define a class variable '_length_cache' of type dict[str, int]")
        
        # retrueve the attribute from the actual subclass, not from the base class, to check its type and contents
        subclass_length_cache = cls.__dict__["_length_cache"]

        # check if it's dict type
        if not type(subclass_length_cache) is dict:
            raise TypeError(f"Subclass {cls.__name__!r} of Entry must define '_length_cache' as a dict[str, int], got {type(subclass_length_cache).__name__!r}")
        ...
        if len(subclass_length_cache) == 0:
            raise ValueError(f"Subclass {cls.__name__!r} of Entry must define a non-empty '_length_cache'")
        
        # check if keys are str and values are int
        for key, value in subclass_length_cache.items():
            if not type(key) is str:
                raise TypeError(f"Keys of '_length_cache' in subclass {cls.__name__!r} of Entry must be of type str, got {type(key).__name__!r}")
            if not type(value) is int:
                raise TypeError(f"Values of '_length_cache' in subclass {cls.__name__!r} of Entry must be of type int, got {type(value).__name__!r} for key {key!r}")
            if value < 0:
                raise ValueError(f"Values of '_length_cache' in subclass {cls.__name__!r} of Entry must be non-negative integers, got {value!r} for key {key!r}")
