...
@dataclass(frozen=True)
class Metadata:
    title: str = "LIST OF WSPHS+ SW FILES AND RELEVANT VERSIONS"
    columns_name: List[str] = field(default_factory=lambda: [
...
class FileVersioningDefaultConfig:
    ...
    version_extraction_criteria: str = r"(?:\\*|//|--|;|#).*Version\s*:?\s*(\d+\.\d+)"
    mode: str = "version"
